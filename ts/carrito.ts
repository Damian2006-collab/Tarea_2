(function () {
  ...
  function addToCart(id: string, name: string, price: number, quantity: number): void {
  ...
  function initQuantityControls(): void {
    document.querySelectorAll<HTMLElement>('.menu-card').forEach((card) => {
      const minusBtn = card.querySelector<HTMLButtonElement>('.qty-btn--minus');
      const plusBtn = card.querySelector<HTMLButtonElement>('.qty-btn--plus');
      const input = card.querySelector<HTMLInputElement>('.qty-input');
      const addBtn = card.querySelector<HTMLButtonElement>('.btn--add-to-cart');
      ...
      input.addEventListener('change', () => {
        ...
        else if (current > MAX_QTY) input.value = String(MAX_QTY);
      });
