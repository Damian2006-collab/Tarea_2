(function () {
  ...
  function getMenuPairs(): MenuPair[] {
    const pairs: MenuPair[] = [];
    document.querySelectorAll<HTMLButtonElement>('.c-menu-account-toggle').forEach((trigger) => {
  ...
  function isMenuOpen(pair: MenuPair): boolean {
    return pair.trigger.getAttribute('aria-expanded') === 'true';
  }

  function openMenu(pair: MenuPair): void {
    pair.menu.hidden = false;
    pair.trigger.setAttribute('aria-expanded', 'true');
  }

  function closeMenu(pair: MenuPair, returnFocusToTrigger: boolean): void {
    pair.menu.hidden = true;
    pair.trigger.setAttribute('aria-expanded', 'false');
    if (returnFocusToTrigger) pair.trigger.focus();
  }
  ...
  function initMenus(): void {
    ...
    document.addEventListener('keydown', (event) => {
      ...
      pairs.forEach((pair) => {
        if (isMenuOpen(pair)) closeMenu(pair, true);
      });
    });
  ...
  function getToastContainer(): HTMLElement | null {
    return document.querySelector<HTMLElement>('.c-toast-container');
  }
  ...
  function showToast(message: string, variant: ToastVariant): void {
    ...
    const existing = container.querySelectorAll<HTMLElement>('.c-toast');
  ...
  function initToastDemoButtons(): void {
